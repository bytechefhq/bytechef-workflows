...
Add a `screenshot.png` file to the folder, along with the `"screenshot"` key in `meta.json`. Existing
templates carry an empty placeholder because the image is not displayed yet, but the file is part of
the folder contract. If you have a real screenshot of the workflow canvas, commit it.
